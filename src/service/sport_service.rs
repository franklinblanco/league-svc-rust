...
use crate::{util::text_serializer, dao::sport_dao};

pub async fn insert_all_sports_from_list(conn: &MySqlPool) {    // This adds a lot of time to the startup. Find a way to cancel it maybe?
    ...
    if persisted_sports.len() == all_sports.len(){ return; }
