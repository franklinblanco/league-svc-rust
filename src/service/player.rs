...
pub async fn edit_player_profile(conn: &MySqlPool, client: &Client, player: PlayerForUpdateDto) -> TypedHttpResponse<Player> {
    ...
    let persisted_player = match unwrap_or_return_handled_error!(player_dao::get_player_with_id(conn, persisted_user.id as u32).await, Player) {
...
pub async fn login(conn: &MySqlPool, client: &Client, mut user: UserForLoginDto) -> TypedHttpResponse<Token> {
    ...
    match unwrap_or_return_handled_error!(player_dao::get_player_with_id(conn, persisted_token.user_id as u32).await, Token) {
...
pub async fn get_player_profile(conn: &MySqlPool, player_id: u32) -> TypedHttpResponse<PlayerProfileDto> {
    let persisted_player = match unwrap_or_return_handled_error!(player_dao::get_player_with_id(conn, player_id).await, PlayerProfileDto) {
        Some(player) => player,
        None => return TypedHttpResponse::return_standard_error(404, MessageResource::new_from_str("Could not find player with id. Something went wrong.")),
    };
    
    let trusted_player_count = unwrap_or_return_handled_error!(trust_dao::get_trusts_by_truster_id(conn, player_id).await, PlayerProfileDto);
    let trusted_by_player_count = unwrap_or_return_handled_error!(trust_dao::get_trusts_by_trustee_id(conn, player_id).await, PlayerProfileDto);
    
    TypedHttpResponse::return_standard_response(200, PlayerProfileDto::new_from_player_and_counts(&persisted_player, trusted_player_count.count, trusted_by_player_count.count))
}

pub async fn get_player_trusted_list(conn: &MySqlPool, player_id: u32) -> TypedHttpResponse<Vec<Player>> {
    match unwrap_or_return_handled_error!(player_dao::get_player_with_id(conn, player_id).await, Vec<Player>) {
        Some(player) => player,
        None => return TypedHttpResponse::return_standard_error(404, MessageResource::new_from_str("Could not find player with id. Something went wrong.")),
    };
...
pub async fn get_player_trusted_by_list(conn: &MySqlPool, player_id: u32) -> TypedHttpResponse<Vec<Player>> {
    match unwrap_or_return_handled_error!(player_dao::get_player_with_id(conn, player_id).await, Vec<Player>) {
        Some(player) => player,
        None => return TypedHttpResponse::return_standard_error(404, MessageResource::new_from_str("Could not find player with id. Something went wrong.")),
    };
