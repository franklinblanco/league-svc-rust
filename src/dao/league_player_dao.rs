...
use league_types::domain::{league_player::LeaguePlayer, enums::league_player_status::LeaguePlayerStatus};
...
pub async fn update_league_player_status(conn: &MySqlPool, league_id: u32, player_id: u32, status: &LeaguePlayerStatus) -> Result<MySqlQueryResult, GenericError<sqlx::Error>> {
    wrap_generic_error_in_wrapper!(sqlx::query_file!("sql/league_player/update.sql", status.to_string(), league_id, player_id).execute(conn).await)
}

pub async fn get_league_player_by_id(conn: &MySqlPool, id: u32) -> Result<LeaguePlayer, GenericError<sqlx::Error>> {
    wrap_generic_error_in_wrapper!(sqlx::query_file_as!(LeaguePlayer, "sql/league_player/get.sql", id).fetch_one(conn).await)
}

pub async fn get_league_players_by_player_id_and_league_id(conn: &MySqlPool, league_id: u32, player_id: u32) -> Result<Vec<LeaguePlayer>, GenericError<sqlx::Error>> {
