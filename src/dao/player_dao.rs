...
pub async fn insert_player(conn: &MySqlPool, player: Player) -> Result<MySqlQueryResult, sqlx::Error>{
    sqlx::query_file!("sql/player/insert.sql", player.id, player.name, player.birth_date, player.country, player.city, player.identification_number, player.bio, player.profile_picture_url, player.id_verified, player.phone_number_verified).execute(conn).await
}
