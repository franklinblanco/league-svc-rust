...
pub async fn get_leagues_by_country_limited_to(conn: &MySqlPool, league_id: i32) -> Result<Vec<League>, GenericError<sqlx::Error>>{
    wrap_generic_error_in_wrapper!(sqlx::query_file_as!(League, "sql/league/get_by_country_limited.sql", league_id, 20).fetch_all(conn).await)
}
