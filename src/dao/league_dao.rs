...
pub async fn get_league_with_id(conn: &MySqlPool, league_id: i32) -> Result<Option<League>, GenericError<sqlx::Error>>{
    wrap_generic_error_in_wrapper!(sqlx::query_file_as!(League, "sql/league/get.sql", league_id).fetch_optional(conn).await)
}

pub async fn get_leagues_by_country_limited_to(conn: &MySqlPool, country: String, from_row: i32, to_row: i32) -> Result<Vec<League>, GenericError<sqlx::Error>>{
...
pub async fn get_leagues_by_in_place_limited_to(conn: &MySqlPool, place_id: i32, from_row: i32, to_row: i32) -> Result<Vec<League>, GenericError<sqlx::Error>>{
...
pub async fn get_leagues_by_player_limited_to(conn: &MySqlPool, player_id: i32, from_row: i32, to_row: i32) -> Result<Vec<League>, GenericError<sqlx::Error>>{
...
pub async fn get_all_leagues_player_has_applied_to(conn: &MySqlPool, player_id: i32, from_row: i32, to_row: i32) -> Result<Vec<League>, GenericError<sqlx::Error>> {
